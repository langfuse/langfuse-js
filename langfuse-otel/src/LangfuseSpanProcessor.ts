...
export class LangfuseSpanProcessor extends BatchSpanProcessor {
  constructor(params: LangfuseSpanProcessorParams) {
    const { publicKey: providedPublicKey, secretKey: providedSecretKey, host: providedHost } = params;
    ...
    const host = providedHost ?? process.env["LANGFUSE_HOST"] ?? "https://cloud.langfuse.com";

    if (!publicKey) throw Error("Missing public key");
    if (!secretKey) throw Error("Missing secret key");

    const basicAuthHeader = "Basic " + Buffer.from(`${publicKey}:${secretKey}`).toString("base64");
