...
```json
{
  "shared": {
    ...
    "devCommand": "pnpm build --watch",
    "devTerminalDescription": "Main development terminal running package builds in watch mode"
  },
  "mcpServers": {
    "playwright": {
      "transport": "stdio",
...
## Why Cursor Compatibility Matters

Cursor users still expect project configuration at the fixed `.cursor/`
discovery paths. This setup preserves that flow by generating `.cursor/mcp.json`
and `.cursor/environment.json` from the shared config instead of tracking
Cursor-specific files as the canonical source.
...
- expand deny lists for new secret-bearing local files that tools should avoid
  reading by default
...
- release or publishing workflows
- package-specific implementation conventions that recur often enough to be
  reusable
